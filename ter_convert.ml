...
let cvrt_const p i_cur =(* print_string("modify\n");*)
    let modify s_n = let rec run_out = function
		      | (e1::l1, t1::q1) -> if e1.signal_name = s_n 
					  then t1
					  else run_out (l1,q1)
		      | _ -> (let rec run_in = function
				|(e2::l2, t2::q2) -> (match t2 with
						 (* | EnumVariantAtom(i) -> i*)
						  | SignalAtom(i) -> if e2.signal_name = s_n 
								      then i
								      else run_in (l2,q2)
						  | _ -> run_in (l2,q2))
				|_ -> failwith "aaaaaah !"
			      in run_in (p.header.signal_declarations.input_signal_list,i_cur.instance_input_expressions))
		      in run_out (p.header.signal_declarations.output_signal_list, i_cur.instance_output_signals)	

	in let rec cvrt = function
		| [] -> []
		| c::l -> (compose_constr c.constraint_kind (modify c.left_signal_name) (modify c.right_signal_name))::(cvrt l)
	    in cvrt p.body.constraint_list
;;
  ...
  match x with 
	...
	| _ -> let rec rep = function
			| (t::l, e::q) -> (*let tst =(e.signal_name = (get_id x)) 
					...
					if e.signal_name = (get_id x) then t else rep (l, q)
			| _ -> failwith "possible ??"
...
let f_IntCst p  a i = [{assigned_signal_name = "Int_cst"; signal_expression = i}];;
...
let replace_assign nv p  i_cur = it_signal_expression p  i_cur
    (f_EnVarAt) 
    (f_SigAt) 
    (f_ClkP) 
    (f_ClkM) 
...
let cvrt_inst (p:process) (i_cur:instantiation) = 
...
(*	match (p.header.signal_declarations.output_signal_list, a.instance_output_signals) with*)
    ...
    in cvrt (p.header.signal_declarations.output_signal_list, i_cur.instance_output_signals)
;;

let cvrt_assign_inst (p:process) (cur_p:process) = 
    let assign_cur = cur_p.body.assignment_list
    and inst_cur = cur_p.body.instantiation_list
    and constr_cur = cur_p.body.constraint_list in
    let rec cvrt = function (* va parcourir les intanciations *)
	| [] -> (constr_cur, assign_cur,[]) (* rend la liste des assign telle qu'elle si vide et la liste vide *)
	| e_c::l_c -> let (r_constr, r_assign, r_inst) = cvrt l_c 
			in if e_c.instance_process_name = p.header.process_name 
				then let n_a = cvrt_inst p e_c 
				      and n_c = cvrt_const p e_c
				      in (n_c@r_constr, n_a@r_assign, r_inst) (* append car il peut y avoir plusieurs sorties, donc, plusieurs résultat *)
				else (r_constr, r_assign, e_c::r_inst)
    in cvrt inst_cur
    
let cvrt_body (p:process) (cur_p:process) = 
    let (constr_l, assign_l, inst_l) = cvrt_assign_inst p cur_p
    in {
		assignment_list = assign_l;
		constraint_list = constr_l;
		instantiation_list = inst_l;
    }
;;

(**************************************************************************************
let cvrt_p_with_lp p lp = {
	header = p.header;
	body = p.body;
};;

let rec cvrt_local_processes p = function
	|[] -> []
	|lp::l -> (cvrt_p_with_lp p lp)::(cvrt_local_processes p l)
;;
**************************************************************)

let cvrt_header h p = {
	process_name = h.process_name;
	signal_declarations = h.signal_declarations ;
	local_process_list = h.local_process_list; 
};;


let cvrt_process (cur_p: process) (p:process) = { 
    header = cvrt_header cur_p.header cur_p;
...
let rec cvrt_processes (p : process list) = match p with
	| [] -> []
	| [e] -> [e]
	| e::l -> let res = (check_processes e l) in 
			if res = l
